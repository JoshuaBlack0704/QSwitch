use ash::vk;
use qvk::{self, init::{instance, device, InstanceFactory, DeviceFactory, DeviceSource}, memory::MemoryFactory, image::{ImageFactory, ImageViewFactory, ImageResourceFactory}, pipelines::graphics::RenderPassAttachment};
...
fn main(){
    ...
    let image = memory.create_image(&device, vk::Format::B8G8R8A8_SRGB, extent, 1, 1, vk::ImageUsageFlags::TRANSFER_SRC, None).unwrap();
