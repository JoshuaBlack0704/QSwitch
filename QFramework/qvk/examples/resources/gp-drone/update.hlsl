...
void main(uint3 GlobalInvocationID : SV_DISPATCHTHREADID){
    ...
    float4 current_pos = i_data[index].current_pos;
    float4 target_pos = i_data[index].target_pos;

    if (ubo.frame == 0){
        float seed = (index + 1);
        uint new_target_index = Random((uint)seed)*ubo.target_count;
        target_pos = target_data[new_target_index];
    ...
    float dist_to = length(target_pos.xyz - current_pos.xyz);
    float travel_dist = velocity * ubo.delta_time;
    if (dist_to < travel_dist){
        current_pos = target_pos;
        float seed = (index + 1) * (current_pos.x + current_pos.y + current_pos.z);
        uint new_target_index = Random((uint)seed)*ubo.target_count;
        target_pos = target_data[new_target_index];
    }   
    else{
        current_pos += float4(dir * travel_dist,1);
    }

    
    i_data[index].target_pos = target_pos;
    i_data[index].current_pos = current_pos;
    float3 start = float3(0,0,1);
    if (length(target_pos - current_pos) == 0){
        dir = float3(1,0,0);
    }


    float4 quat = from_to_rotation(start, dir);
    float4x4 rotation = quaternion_to_matrix(quat);

    float4x4 translation = {
        1,0,0,current_pos.x,//row1
        0,1,0,current_pos.y,//row2
        0,0,1,current_pos.z,//row3
        0,0,0,1//row4
    };

    float4x4 mvp = mul(ubo.projection, mul(ubo.view, mul(translation, rotation)));
    //i_data[index].current_pos += ubo.delta_time * 1.0;

    i_data[index].mvp_matrix = mvp;
}