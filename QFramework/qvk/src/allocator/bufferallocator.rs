...
use crate::{init::{DeviceSource, InstanceSource}, allocator::{PartitionSystem, test_partition}};

use super::{MemorySource, BufferAllocatorFactory, BufferAllocator, BufferExtensions, BufferSource, MemPart, BufPart};
...
impl<M:DeviceSource + MemorySource> BufferSource for Arc<BufferAllocator<M>>{
    fn get_space(&self, size: u64, alignment: Option<u64>) -> (MemPart, BufPart) {
        ...
        if let Some(indices) = &self.share{
            info = info.sharing_mode(vk::SharingMode::CONCURRENT).queue_family_indices(indices);
        }
        ...
        let mp = self.mem_alloc.get_space(mem_reqs.size, Some(mem_reqs.alignment));
...
//                return BufferSegment::new(self.clone(), mem.clone(), (*b,p)); 
//             }
//         }
...
impl BufferExtensions{
    pub fn push<'a>(&mut self, _builder: vk::BufferCreateInfoBuilder<'a>) -> vk::BufferCreateInfoBuilder<'a>{
        todo!()
    }
}
