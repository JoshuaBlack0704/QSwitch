...
pub trait RenderpassFactory<R:RenderPassSource, A:RenderpassAttachmentSource>{
    fn create_renderpass<S:SubpassDescriptionSource>(&self, attachments: &[&A], subpasses: &[&S], flags: Option<vk::RenderPassCreateFlags>) -> R;
}
...
pub trait GraphicsPipelineFactory<G:GraphicsPipelineSource, S:GraphicsPipelineState, L:PipelineLayoutSource, R:RenderPassSource>{
    fn create_graphics_pipeline(&self, state: &S, layout: &L, renderpass: &R, tgt_subpass: u32) -> Result<G, vk::Result>;
}
