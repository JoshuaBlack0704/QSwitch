...
use crate::memory::{partitionsystem, Memory, MemorySource, PartitionSystem};

use super::{Image, ImageFactory};

impl<D: DeviceSource + Clone, M: DeviceSource + MemorySource + Clone>
    ImageFactory<D, Arc<Image<D, M>>> for M
{
    fn create_image(
        &self,
        device_source: &D,
    ...
    ) -> Result<Arc<Image<D, M>>, ImageCreateError> {
        ...
        let res =
            unsafe { device.bind_image_memory(image, *self.memory(), memory_partition.offset) };
        if let Err(e) = res {
            return Err(ImageCreateError::Vulkan(e));
        }
...
impl<D: DeviceSource + Clone, M: MemorySource + DeviceSource + Clone> Image<D, M> {
    ...
    ) -> Arc<Image<D, Arc<Memory<D, PartitionSystem>>>> {
...
impl<D: DeviceSource + Clone, M: MemorySource + DeviceSource> ImageSource for Arc<Image<D, M>> {
    fn transition<C: CommandBufferSource>(
        ...
        if let Some(stage) = src_stage {
            ...
            image_transition =
                image_transition.src_stage_mask(vk::PipelineStageFlags2::TOP_OF_PIPE);
        }
        ...
        if let Some(stage) = dst_stage {
            ...
            image_transition =
                image_transition.dst_stage_mask(vk::PipelineStageFlags2::BOTTOM_OF_PIPE);
        }
        ...
        let info = vk::DependencyInfo::builder().image_memory_barriers(&image_transition);

        debug!(
            "Transitioning layer range {:?} of image {:?} from layout {:?} to layout {:?}",
            image_transition[0].subresource_range, self.image, *lock, new_layout
        );
...
impl<D: DeviceSource + InstanceSource, M: MemorySource + DeviceSource> InstanceSource
    for Arc<Image<D, M>>
{
...
impl<D: DeviceSource, M: MemorySource + DeviceSource> MemorySource for Arc<Image<D, M>> {
    ...
    ) -> Result<crate::memory::Partition, partitionsystem::PartitionError> {
