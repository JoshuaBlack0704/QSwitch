...
impl<Img: ImageSource + DeviceSource + InstanceSource + Clone>
    ImageResourceFactory<Arc<ImageResource<Img>>> for Img
{
    fn create_resource(
        &self,
        offset: vk::Offset3D,
    ...
    ) -> Result<Arc<ImageResource<Img>>, ImageResourceCreateError> {
        ...
        if req_size.width > act_size.width
            || req_size.height > act_size.height
            || req_size.depth > act_size.depth
        {
            return Err(ImageResourceCreateError::ResourcesDontExist);
        }
...
impl<Img: ImageSource + DeviceSource + InstanceSource + Clone> ImageResourceSource
    for Arc<ImageResource<Img>>
{
    ...
    fn blit_to_image_internal<IR: ImageCopyFactory>(
        &self,
        dst: &IR,
...
impl<Img: ImageSource + DeviceSource + InstanceSource> ImageCopyFactory
    for Arc<ImageResource<Img>>
{
    fn extent(&self) -> vk::Extent3D {
        self.extent
...
impl<Img: ImageSource + DeviceSource + InstanceSource> ImageTransitionFactory
    for Arc<ImageResource<Img>>
{
    fn image(&self) -> vk::Image {
        *self.image.image()
