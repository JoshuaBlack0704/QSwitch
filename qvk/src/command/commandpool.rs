...
use super::{
    CommandBuffer, CommandBufferFactory, CommandBufferSource, CommandPool, CommandPoolFactory,
    CommandPoolOps,
};
use crate::init::DeviceSource;

impl<D: DeviceSource + Clone> CommandBufferFactory<Arc<CommandBuffer<D>>>
    for Arc<CommandPool<D, Arc<CommandBuffer<D>>>>
{
    fn next_cmd(&self, level: vk::CommandBufferLevel) -> Arc<CommandBuffer<D>> {
        ...
        let new_cmds = unsafe {
            self.device
                .device()
                .allocate_command_buffers(&alloc_builder)
    ...
    fn reset_cmd(
        &self,
        cmd: &Arc<CommandBuffer<D>>,
        reset_flags: Option<vk::CommandBufferResetFlags>,
    ) {
        if let Some(f) = reset_flags {
            unsafe { self.device.device().reset_command_buffer(cmd.cmd(), f) }
                .expect("Failed to reset command buffer");
        } else {
            unsafe {
                self.device
                    .device()
                    .reset_command_buffer(cmd.cmd(), vk::CommandBufferResetFlags::empty())
            }
            .expect("Failed to reset command buffer");
        }
    }
...
impl<D: DeviceSource + Clone> CommandPoolFactory<Arc<CommandPool<D, Arc<CommandBuffer<D>>>>> for D {
    fn create_command_pool(
    ...
    ) -> Result<Arc<CommandPool<D, Arc<CommandBuffer<D>>>>, vk::Result> {
        ...
        let command_pool = unsafe {
            device_provider
                .device()
                .create_command_pool(&cmdpool_cinfo, None)
        };

        match command_pool {
            Ok(pool) => {
                info!("Created command pool {:?}", pool);
                return Ok(Arc::new(CommandPool {
        ...
        match self.reset_flags {
            Some(f) => {
                unsafe {
                    self.device
                        .device()
                        .reset_command_pool(self.command_pool, f)
                }
...
impl<D: DeviceSource + InstanceSource, C: CommandBufferSource> InstanceSource
    for Arc<CommandPool<D, C>>
{
